...
function NodeRow({ row, onView, onEdit, onServiceArea, onDelete }) {
  ...
  return <tr className="hover:bg-gray-50/50 transition-colors bg-white"><td className="px-6 py-4"><div className="flex items-center gap-4"><div className="w-10 h-10 rounded-full bg-red-50 flex items-center justify-center text-[#E31837] shrink-0"><RowIcon className="w-5 h-5" /></div><div><p className="font-bold text-gray-900 text-sm">{row.name}</p><p className="text-xs text-gray-500 font-medium">{row.code}</p></div></div></td><td className="px-6 py-4"><span className="inline-flex px-2.5 py-1 rounded-md text-xs font-bold text-[#E31837] bg-red-50">{NODE_TYPE_LABELS[row.type] || row.type}</span></td><td className="px-6 py-4 whitespace-normal max-w-xs"><div className="flex items-start gap-2 text-gray-600"><MapPin className="w-4 h-4 text-gray-400 shrink-0 mt-0.5" /><span className="font-medium text-sm leading-snug">{nodeLocation(row)}</span></div></td><td className="px-6 py-4"><span className="font-bold text-gray-900">{formatNumber(row.dailyCapacity)}</span></td><td className="px-6 py-4"><div className="space-y-1"><div className="flex items-center gap-2 text-gray-700 text-sm"><User className="w-3.5 h-3.5 text-gray-400" /><span className="font-medium">{row.contactName || (row.managerUserId ? `Manager #${row.managerUserId}` : '-')}</span></div><div className="text-gray-500 text-xs font-medium">{row.contactPhone || '-'}</div></div></td><td className="px-6 py-4"><StatusBadge active={row.active} /></td><td className="px-6 py-4"><div className="flex items-center justify-center gap-2"><IconButton title="View" onClick={() => onView(row.id)} icon={Eye} /><IconButton title="Edit" onClick={() => onEdit(row.id)} icon={Edit} /><IconButton title="Service Area" onClick={() => onServiceArea(row)} icon={MapPin} /><IconButton title="Delete" onClick={() => onDelete(row)} icon={Trash2} danger /></div></td></tr>;
}
...
function NodeDrawer({ mode, form, setForm, node, onClose, onSave }) {
  ...
  return <Modal open={open} onClose={onClose} title={mode === 'node-new' ? 'Add Hub/Branch' : mode === 'node-edit' ? 'Edit Hub/Branch' : 'View Hub/Branch'} size="lg" footer={readOnly ? <button className="px-4 py-2 bg-[#111111] text-white font-medium rounded-lg" onClick={onClose}>Close</button> : <><button className="px-4 py-2 border border-gray-200 text-gray-700 font-medium rounded-lg" onClick={onClose}>Cancel</button><button className="px-4 py-2 bg-[#111111] text-white font-medium rounded-lg" onClick={onSave}>Save Changes</button></>}>
...
function RouteDrawer({ mode, form, setForm, route, nodeOptions, onClose, onSave }) {
  ...
  return <Modal open={open} onClose={onClose} title={mode === 'route-new' ? 'Add Route' : mode === 'route-edit' ? 'Edit Route' : 'View Route'} size="md" footer={readOnly ? <button className="px-4 py-2 bg-[#111111] text-white font-medium rounded-lg" onClick={onClose}>Close</button> : <><button className="px-4 py-2 border border-gray-200 text-gray-700 font-medium rounded-lg" onClick={onClose}>Cancel</button><button className="px-4 py-2 bg-[#111111] text-white font-medium rounded-lg" onClick={onSave}>Save Changes</button></>}>
